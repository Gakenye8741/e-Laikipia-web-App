...
export const DeansAppeals = () => {
  ...
  const handleViewEvidence = (url: string) => {
    MySwal.fire({
      title: '<span class="text-[10px] font-black uppercase tracking-widest text-slate-400">Dean\'s Evidence Review</span>',
      imageUrl: url,
  ...
  const handleDecision = async (appealId: string, status: "APPROVED" | "REJECTED") => {
    ...
    const { value: comment, isConfirmed } = await MySwal.fire({
      ...
      text: isApprove 
        ? "Confirming this will clear the student's disciplinary or status record for this application." 
        : "Provide a justification for upholding the rejection.",
      input: 'textarea',
      inputPlaceholder: 'e.g. Conduct review cleared / Insufficient evidence provided.',
      showCancelButton: true,
      confirmButtonText: isApprove ? 'APPROVE NOW' : 'REJECT APPEAL',
      confirmButtonColor: isApprove ? '#4f46e5' : '#e11d48',
  ...
  if (isLoading) return (
    <div className="flex flex-col justify-center items-center h-screen bg-[#F8FAFC]">
      ...
      <p className="mt-4 text-[10px] font-black text-slate-400 uppercase tracking-widest">Accessing Student Affairs Registry...</p>
    </div>
  );
    ...
    <div className="p-8 bg-[#F8FAFC] min-h-screen font-sans">
      <div className="max-w-6xl mx-auto">
        ...
        <div className="mb-10 flex flex-col lg:flex-row lg:items-end justify-between gap-6">
          <div>
            <div className="inline-flex items-center gap-2 px-3 py-1 bg-indigo-50 rounded-full mb-3 border border-indigo-100">
              <span className={`h-2 w-2 rounded-full bg-indigo-600 ${isFetching ? 'animate-ping' : ''}`}></span>
              <span className="text-[10px] font-black text-indigo-700 uppercase tracking-widest">Dean of Students Office</span>
            </div>
            <h1 className="text-4xl font-black text-slate-900 tracking-tighter uppercase italic leading-none">
              Student <span className="text-indigo-600">Appeals</span>
            </h1>
          </div>
          ...
          <div className="flex flex-wrap gap-3">
            <div className="relative">
              <input 
                ...
                className="bg-white border border-slate-200 rounded-2xl p-3 pl-10 text-xs font-bold outline-none focus:ring-2 focus:ring-indigo-500 w-64 shadow-sm" 
              />
            ...
            <div className="flex bg-slate-200/50 p-1 rounded-2xl">
              <button 
                ...
                className={`px-4 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${activeTab === 'PENDING' ? 'bg-white text-indigo-700 shadow-sm' : 'text-slate-500'}`}
              >
            ...
            <button onClick={() => refetch()} className={`p-3 bg-white border border-slate-200 rounded-2xl hover:bg-indigo-50 transition-all shadow-sm ${isFetching ? 'animate-spin' : ''}`}>
                ...
                {paginatedAppeals.length > 0 ? (
                    ...
                    <tr key={appeal.id} className="hover:bg-slate-50/80 transition-colors group">
                      ...
                      <td className="px-8 py-6 max-w-xs">
                        ...
                        {appeal.reviewer_comment && (
                           <div className="mt-2 flex items-center gap-1">
                             <span className="text-[8px] font-black bg-indigo-100 text-indigo-700 px-2 py-0.5 rounded-md uppercase">Dean's Remark</span>
                      ...
                      <td className="px-8 py-6 text-center">
                        <div className={`inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-[8px] font-black uppercase tracking-widest ${
                          ...
                          appeal.status === 'APPROVED' ? 'bg-indigo-100 text-indigo-600' : 'bg-red-100 text-red-600'
                        }`}>
                          <span className={`h-1.5 w-1.5 rounded-full ${
                             appeal.status === 'PENDING' ? 'bg-amber-500' : 
                             appeal.status === 'APPROVED' ? 'bg-indigo-500' : 'bg-red-500'
                          }`}></span>
                          {appeal.status}
                        </div>
                      </td>
                  ...
                  <tr>
                    <td colSpan={4} className="py-24 text-center">
                       ...
                       <p className="text-[11px] font-black text-slate-400 uppercase tracking-widest">No welfare appeals found</p>
                    </td>
                  </tr>
                )}
              </tbody>
              ...
              <div className="flex gap-1.5">
                {[...Array(totalPages)].map((_, i) => (
                  <button 
                    ...
                    className={`w-9 h-9 rounded-xl text-[10px] font-black transition-all ${currentPage === i + 1 ? 'bg-indigo-600 text-white shadow-indigo-200 shadow-lg' : 'bg-white border border-slate-200 text-slate-400 hover:border-indigo-300'}`}
                  >
                    {i + 1}
                  </button>
                ))}
